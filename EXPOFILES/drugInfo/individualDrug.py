...
def individualDrug(UIController: UIController, medName: str) -> None:
    ...
    dosage_text = tk.Label(UIController.canvas, text=f'{site_info["dosage_text"]}', bg=PRIMARY_COLOR, font=(TEXT_FONT, 20, 'normal'), wraplength=750)
    meta_label = tk.Label(UIController.canvas, text="Sourced from Drugs.com", bg=PRIMARY_COLOR, font=(TEXT_FONT, 15, 'normal'))
    ...
    if image_path:
        ...
        image_text = tk.Label(UIController.canvas, text=f'Pill of {medName}', bg=PRIMARY_COLOR, font=(TEXT_FONT, 20, 'normal'))

        UIController.canvasIds["DrugInfo"].append(UIController.canvas.create_window(WINDOW_WIDTH - (WINDOW_PADDING * 2), WINDOW_HEIGHT / 1.5, window=image_text, anchor=tk.E))
        UIController.canvasIds["DrugInfo"].append(UIController.canvas.create_window(WINDOW_WIDTH_PADDING, WINDOW_HEIGHT / 2, window=report_label, anchor=tk.E))
